...
class NeteasePublisher(BasePublisher):
    ...
    发布页面：https://mp.163.com/upload/article
    """
            ...
            for selector in selectors:
                try:
                    if await self.wait_for_selector(page, selector, 5000):
                        ...
                        logger.info(f"网易号标题已填充: {title[:20]}...")
                        return True
                except Exception:
                    continue
    ...
    async def _fill_content(self, page: Page, content: str) -> bool:
        ...
        try:
            ...
            for selector in selectors:
                try:
                    if await self.wait_for_selector(page, selector, 5000):
                        ...
                        logger.info(f"网易号正文已填充: {len(content)} 字符")
                        return True
                except Exception:
                    continue
            ...
            logger.info(f"网易号正文已填充（直接输入）: {len(content)} 字符")
            return True

        except Exception as e:
            logger.error(f"网易号正文填充失败: {e}")
            return False
